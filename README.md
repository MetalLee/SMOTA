...
- `tasks` 表是任务分解和任务进度的唯一事实来源；`ROADMAP.md` 只作为可读说明。只有分配给 CodingAgent 的任务会传入 OpenCode，并由 CodingAgent 通过受控 HTTP API 回写任务状态；其他任务继续按分配的 Agent 状态展示。
...
12. Sandbox 执行 OpenCode CLI；CodingAgent 只按分配给 CodingAgent 的 task id 执行，并用 HTTP 请求把这些任务状态回写到 `tasks.status`。
13. Sandbox 执行依赖安装和构建；失败时执行一次自动修复。
14. stdout / stderr、构建状态、错误和 step 事件持续写入 `run_events`。
15. Runner 扫描 `/workspace` 文件树并写入 `workspace_files`。
16. Vite dev server 以 `5173` 端口启动，preview URL 写入 `agent_runs` 和 `sandbox_runs`。
17. ReviewerAgent 基于构建结果、事件、文件索引和已知问题生成中文 `REVIEW_REPORT.md`。
...
- PlannerAgent 生成的 `tasks` 是工作台任务清单和进度跟踪的唯一来源；CodingAgent 任务状态由 OpenCode 回写，其他任务继续跟随对应 Agent 状态。
- 用户批准计划后自动启动 Vercel Sandbox。
- Sandbox 内写入 Harness、初始化 Vite React TypeScript 应用、执行 OpenCode CLI。
- CodingAgent Prompt 只包含分配给 CodingAgent 的 task id 和任务状态回写协议，OpenCode 完成任务后通过 HTTP API 更新这些任务的 `tasks.status`。
...
SMOTA_APP_URL=https://your-app.example.com
...
`NEXT_PUBLIC_*` 会暴露给浏览器。`SUPABASE_SERVICE_ROLE_KEY`、模型密钥和 Vercel token 只能配置在服务端环境中，不能传入前端或 Sandbox。Sandbox 只接收 OpenCode 运行所需的最小环境变量白名单，以及当前 Run 专用的 `SMOTA_TASK_UPDATE_URL` / `SMOTA_TASK_UPDATE_TOKEN`。`SMOTA_TASK_UPDATE_SECRET` 用于服务端生成任务回写 token；未配置时使用服务端 `SUPABASE_SERVICE_ROLE_KEY` 派生 token，但仍不会把 service role key 注入 Sandbox。
