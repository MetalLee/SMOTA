...
-- supabase/migrations/0006_project_sharing.sql
-- supabase/migrations/0007_continuation_runs.sql
```
...
- 当项目当前 Run 已完成或失败时，左侧 Agent Panel 底部输入框会启用。
- 用户输入新的修改提示后，平台会创建同一项目下的新 Run；普通项目会记录 `parent_run_id`，克隆项目会根据已有 `sandbox_name` 和 `workspace_files` 解析可复用 workspace。
- ProductAgent、ArchitectAgent 和 PlannerAgent 会生成增量 Harness，不会把项目当作全新需求处理。
- 批准后 Runner 复用已有持久化 Sandbox 和 `/workspace`，跳过 Vite 初始化，直接执行 OpenCode 增量修改、安装、构建、一次自动修复和 ReviewerAgent。
- 如果当前项目没有可复用 Sandbox 文件，系统会提示重新生成或重新克隆。
...
发现页和收藏页只展示最新 Run 处于终态的共享项目。若项目正在执行新的继续开发 Run，例如处于规划、待批准、已批准、运行中或可重试状态，即使上一轮已有 preview URL，也会暂时从“发现”和“收藏”列表隐藏，直到最新 Run 完成或失败。
