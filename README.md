...
- `tasks` 表是任务分解和任务进度的唯一事实来源；`ROADMAP.md` 只作为可读说明。只有分配给 CodingAgent 的任务会传入 OpenCode，并由 CodingAgent 通过受控 HTTP API 回写任务状态；BuildAgent 构建成功后由平台服务端兜底将 BuildAgent 任务更新为完成；其他任务继续按分配的 Agent 状态展示。
...
- PlannerAgent 生成的 `tasks` 是工作台任务清单和进度跟踪的唯一来源；CodingAgent 任务状态由 OpenCode 回写，BuildAgent 任务在构建成功后由 Runner 兜底更新为完成，其他任务继续跟随对应 Agent 状态。
- 用户批准计划后自动入队 Vercel Sandbox workflow，启动接口快速返回，构建进度通过 Supabase 状态和事件刷新。
- Sandbox 内写入 Harness、初始化 Vite React TypeScript 应用、执行 OpenCode CLI。
- 生成应用固定使用 `/workspace` 作为 Vite 根目录，禁止再创建项目名子目录作为应用根。
