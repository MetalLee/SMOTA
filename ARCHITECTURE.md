...
Runner 负责创建和控制 Vercel Sandbox。它写入 Harness 文件、初始化生成应用、注入仅运行时使用的密钥、调用 Codex CLI、执行包管理命令、暴露 dev server 端口，并把状态回写到 Supabase。
...
- Codex CLI 所需 API Key 只在 Sandbox 执行时注入。
...
4. `generating`：Codex CLI 或 agents 正在修改代码。
5. `installing`：正在安装依赖。
6. `building`：正在执行构建命令。
...
- `CODEX_API_KEY`
- `SANDBOX_DEFAULT_RUNTIME=node24`
- `SANDBOX_DEFAULT_TIMEOUT_MS`
- `SANDBOX_PREVIEW_PORT=5173`
...
- Codex CLI 只在 Vercel Sandbox 内执行。
- 所需 API key 在命令执行时注入 Sandbox 环境。
- Codex CLI 使用项目 Harness 文件作为本地上下文。
- Codex CLI 必须将变更写入 `/workspace`。
- Codex CLI 输出流式写入 `run_events`。
