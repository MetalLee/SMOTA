...
`tasks` 表是计划任务和执行进度的唯一事实来源。`ROADMAP.md` 只作为用户可读的计划说明，不能作为独立任务状态来源；`agent_runs` 表示整体运行生命周期。只有分配给 CodingAgent 的任务由 CodingAgent 通过 HTTP 回写 `tasks.status` 并与 Agent 运行状态解绑；BuildAgent 构建成功后由平台服务端兜底将当前 Run 下 `agent_name = BuildAgent` 的任务更新为 `done`；其他任务仍按 `agent_name` 对应 Agent 的运行状态展示进度。
...
12. 在 Sandbox 内执行 OpenCode CLI。
13. CodingAgent 只接收 `tasks` 表中 `agent_name = CodingAgent` 的任务 ID，并通过受控 HTTP API 回写这些任务的 `tasks.status`；Prompt 明确禁止 CodingAgent 更新 BuildAgent、ReviewerAgent 或其他规划 Agent 的任务。
14. 在 Sandbox 内执行 `pnpm install` 和 `pnpm build`。
15. 如果构建失败，执行一次自动修复。
16. 运行日志、状态和构建输出写入 `run_events`。
17. 扫描文件树并持久化到 Supabase。
18. 在 Sandbox 内启动 `pnpm dev --host 0.0.0.0 --port 5173`。
19. 保存 preview URL，并在 Web Console 的 iframe 中展示。
...
- `current_agent text`
- `current_step text`
- `planning_generation integer default 0`
- `input_prompt text`
...
`status` 允许值由服务端代码约束为 `todo`、`in_progress`、`done`、`failed`。CodingAgent 只能通过 `POST /api/runs/[runId]/tasks/[taskId]/status` 更新当前 Run 下 `agent_name = CodingAgent` 的任务；该接口只更新 `tasks.status` 并写入 `run_events`，不更新 `agent_runs.status`。非 CodingAgent 任务不传入 OpenCode，工作台继续根据对应 Agent 状态展示进度；BuildAgent 构建成功时，Sandbox Runner 会额外把当前 Run 下 `agent_name = BuildAgent` 且尚未完成的任务批量更新为 `done` 并写入 `task.status.updated` 事件。
...
- 将 `/workspace` 作为唯一 Vite 应用根目录；不得在 `/workspace` 下再创建 `gomoku/`、`todo-app/`、`app/` 等项目名子目录作为应用根。
...
2. `queued`：Sandbox workflow job 已入队，等待后台 worker claim。
3. `creating`：服务端正在创建 Vercel Sandbox。
4. `ready`：Sandbox 已创建，可以写入文件和执行命令。
5. `generating`：OpenCode CLI 或 agents 正在修改代码。
6. `installing`：正在安装依赖。
7. `building`：正在执行构建命令。
8. `fixing`：构建失败后正在执行一次自动修复。
9. `previewing`：dev server 已运行，preview URL 可用。
10. `failed`：运行失败，无法继续自动处理。
11. `stopped`：Sandbox 已停止或过期。

## Sandbox 文件读写方式

- 服务端 API 将 Harness 文件写入 `/workspace`。
- 生成应用的文件保留在 Sandbox 文件系统内。
- `index.html`、`package.json`、`src/`、`public/`、`vite.config.ts` 等应用入口和源码必须位于 `/workspace` 根或标准子目录内，不能被规划或生成到项目名子目录中。
...
- ReviewAgent 只在生成应用 `pnpm build` 成功且 dev server 已启动后截图，不能对 `init_vite` 后的默认 Vite Home 截图；AgentRun 只能在 Review report 写入完成后进入 `succeeded`，不能在截图或报告仍运行时提前标记完成。
...
- 截图是可选质量检视增强，默认总命令超时由 `PREVIEW_SCREENSHOT_COMMAND_TIMEOUT_MS` 限制，避免可选截图拖垮 Sandbox workflow；截图失败只写入 `review.screenshot.failed` 事件，不应把已成功的 build 标记为失败。
...
- `PREVIEW_SCREENSHOT_COMMAND_TIMEOUT_MS=120000`
...
- CodingAgent Prompt 必须明确禁止更新非 CodingAgent 任务；BuildAgent 任务由平台服务端在构建成功后兜底更新为 `done`。
