...
模型流式返回中的 `reasoning_content` 只作为可审计进度摘要写入 `run_events`，事件类型为 `agent.reasoning`。Web 工作台左侧 Agent Panel 可展示这些事件；不要把系统提示词、服务端密钥或 Supabase service role key 写入 Sandbox 或前端。
