...
允许已完成、已失败或克隆得到的项目基于当前 Sandbox 文件继续开发，而不是从空项目重新生成。

### 任务

- 在终态 Run 的左侧 Agent Panel 启用继续开发输入框。
- 为同一项目创建新的 AgentRun，并用 `parent_run_id` 记录普通续跑关系。
- 对克隆项目支持无语义父 Run 的 workspace 解析，只要存在 Sandbox 文件索引即可继续开发。
- 让 ProductAgent、ArchitectAgent 和 PlannerAgent 基于原始需求、本次修改提示、已有 Harness 和当前文件索引生成增量计划。
- 批准后复用已有 Sandbox 和 `/workspace`，跳过 Vite 初始化，执行 OpenCode 增量修改、安装、构建、一次修复和 ReviewerAgent。

### 验收标准

- 已完成或已失败的项目可以再次输入修改提示并生成新 Run。
- 克隆项目首次继续开发也基于克隆得到的文件修改。
- 继续开发不会覆盖已有应用骨架。
- 没有可复用 Sandbox 文件时展示清晰中文错误。
